...
function Chats({author, onRoomCreate, pathname}) {
	...
	useEffect( () => {
		if (pathname !== '' && rooms.includes(pathname) && !connectedRooms.includes(pathname)){
	...
	return (
		<div className="Chats-main">
			<Chats-Head>
				...
				<ul className="Chats-existed-rooms"> Existed rooms - click name to join
					{rooms.map( (room) => <li key={room} onClick={onRoomNameClick} >{room}</li>)}
				</ul>
			</Chats-Head>
			<div className="Chats-chats">
				{connectedRooms.map(room =><ChatWindow key={room} author={author} roomName={room}></ChatWindow>)}
			</div>
		</div>
	)
}
